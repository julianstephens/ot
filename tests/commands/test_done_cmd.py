...
def test_done_success(mock_storage, mocker):
    ...
    assert "Commitment for 2023-01-01 completed: Test Commitment" in result.stdout
    mock_storage.complete_day.assert_called_once_with(None)


def test_done_with_date(mock_storage, mocker):
    mock_day = mocker.MagicMock()
    mock_day.title = "Test Commitment"
    mock_storage.complete_day.return_value = ("2023-01-01", mock_day)

    result = runner.invoke(app, ["done", "--date", "2023-01-01"])

    assert result.exit_code == 0
    mock_storage.complete_day.assert_called_once_with("2023-01-01")


def test_done_not_initialized(mock_storage):
...
def test_done_day_unset_no_prompt(mock_storage):
...
def test_done_day_unset_prompt(mock_storage, mocker):
    mock_storage.complete_day.side_effect = DayUnsetError
    mock_storage.settings.auto_prompt_on_empty = True
    mock_prompt = mocker.patch("ot.commands.done_cmd.prompt_set_commitment")
...
def test_done_already_done(mock_storage):
...
def test_done_generic_error(mock_storage):
    mock_storage.complete_day.side_effect = Exception("Boom")

    result = runner.invoke(app, ["done"])

    assert result.exit_code == 1
    assert "Error marking commitment as done: Boom" in result.stdout
